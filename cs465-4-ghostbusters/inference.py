...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        newParticles = []
        if noisyDistance is None:                #only one ghost present so noisy ditance 0 means ghost is eaten so put
            for i in range(0, self.numParticles):
                newParticles.append(self.getJailPosition())
            self.particles = newParticles

        else:                                   #see how each particle matches the evidence...
            weightedParticles = util.Counter()
            for p in self.particles:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                weightedParticles[p] += emissionModel[trueDistance]

            if weightedParticles.totalCount() == 0:     #if all particles are 0
                self.initializeUniformly(gameState)

            else:                                        #Resampling  done    
